...
Private Sub Kursatish_Click()
    avFiles = Application.GetOpenFilename("Excel files(*.xls*),*.xls*", 1, "Âûáðàòü Excel ôàéëû", , False)
...
Private Sub OK_Click()
    ...
    ListManba = Array("ÖÁ", "ÊÁ")
    bListlar = Array(ÖÁ, ÊÁ)
    ...
    If Len(Yul) = 0 Then
        ...
        Exit Sub
    End If
    If ÖÁ.Value = False And ÊÁ.Value = False Then
    ...
    OldingiKunTest = wbJamlovchi.Worksheets("ÖÁ(êîíñ_new)").Rows(4).Cells.Find(What:=CDate(tbOldingiSana), LookIn:=xlFormulas).Column
    If Err <> 0 Then
...
'    Yul = "D:\Ìîíåòàð ñè¸ñàò\ÄÊÏ\Áàëàíñ\Áàëàíñ_ÖÁ_ÊÁ_êóíëèê (ìèíã)_17.08.xls"
    
'    Set joriy_sana = Workbooks("Áàëàíñ ìàêðî.xlsm").Sheets(1).Range("A1")
'    new_sana = DateSerial(Year(joriy_sana), Month(joriy_sana) + 2, 0)
'    Yul = "D:\Balance\áàëàíñ 2014-2019\" & Format(new_sana, "YYYY-MM-DD") & ".xls"
