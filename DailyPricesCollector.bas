...
Public Const stPath$ = "C:\Users\msd15\Desktop\Êóíëèê íàðõëàð\"    'fayllar saqlanadigan papka
Const sServerName$ = "Main/ABM", sUserDB$ = "d_ext\mb\01msd14.nsf" 'lokal server va ma'lumotlar bazasi nomlari
                                                                   'Lotus => Ôàéë => Áàçà äàííûõ => Ñâîéñòâà (Ñåðâåð, Èìÿ ôàéëà)
Const Shablon$ = "C:\Users\msd15\Desktop\Êóíëèê íàðõëàð\+Ïå÷àòãà Êóíëèê Øàáëîí.xlsx"    'Kun ochish u-n shablon fayl
...
NewFolder = "Íàðõëàð êóíëèê\" & fldrMonthName & "\" & fldrDateName
    ...
    If Doc.HasEmbedded Then
        ...
        If vaItem.Type = RICHTEXT Then
            For Each vaAttachment In vaItem.EmbeddedObjects
                ...
                If InStr(1, Filename, "êóíëèê", 3) <> 0 Then
...
PechatDir = "C:\Users\msd15\Desktop\Êóíëèê íàðõëàð\!Ïå÷àòü\" & fldrMonthName
...
With JamlovchiWorkBk.Sheets("Êóíëèê íàðõëàð")
...
Do While sFiles <> ""
      If sFiles <> JamlovchiWorkBk.Name Or InStr(1, sFiles, "êóíëèê", 3) <> 0 Then 'check if the names of files include the specified word
        ...
        If Err <> 0 Then
          ...
          Do
            InvalidChar = WorksheetFunction.Find(Chr(63), sFiles, i)
                If i > 1 Then
                    sNewFileName = WorksheetFunction.Replace(sNewFileName, InvalidChar, 1, "ê")
                    sFileName = WorksheetFunction.Replace(sFileName, InvalidChar, 1, ChrW(1178))
                Else
                    sNewFileName = WorksheetFunction.Replace(sFiles, InvalidChar, 1, "ê")
        ...
        Hudud = Left(Cells(2, 2).Value, 5)
            ...
            Else
                Select Case Hudud
                    ...
                    Case "Ñèðäà": h = 8
                    Case "Ñóðõî": h = 9
                    Case "ÔÀÐ" & ChrW(1170) & "Î": h = 11
                    Case ChrW(1178) & "àø" & ChrW(1179) & "à": h = 12
                    Case "Õîðàç": h = 13
                    Case Else: h = 1
            End Select
            End If
        ...
        Sheets("Êóíëèê íàðõëàð").Range(Sheets("Êóíëèê íàðõëàð").Cells(5, h + 3), Sheets("Êóíëèê íàðõëàð").Cells(LastRow, h + 3)).Value = TempArray
        '**************
        Sheets("Êóíëèê íàðõëàð").Cells(99, h + 3) = fldrtime(h)
        '**************
     End If
    sFiles = Dir
    Call MyProgresBar
...
JamlovchiWorkBk.Worksheets("Ïå÷àòãà").Activate
JamlovchiWorkBk.SaveAs PechatDir & "\" & "+Ïå÷àòãà Êóíëèê " & Format(tdate, "dd.mm.yyyy") & ".xlsx"
Set TayyorWb = ActiveWorkbook
TayyorWb.Save
Call MyProgresBar

'Create a new memo in Lotus Notes
If WorksheetFunction.Count(Sheets("Êóíëèê íàðõëàð").Range("D5:Q5")) = 14 Then
    Dim EditDoc As Object, AttachME As Object, Attachment$, EmbedObj
    
    Set NewDoc = noDatabase.CREATEDOCUMENT
    With NewDoc
        .form = "Memo"
        .Subject = "Êóíëèê íàðõëàð " & fldrDateName & " õîëàòèãà"
...
With Application
 ...
 .DisplayAlerts = True
 Finish = Timer
 .StatusBar = Format(Finish - Start, "0.00") & " ñåêóíääà ìóâàôôà" & ChrW(1179) & "èÿòëè áàæàðèëäè!"
